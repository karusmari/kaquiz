...
##### Ask the student to show you the implementation of the `backend` and `frontend` for
the project.
...
##### Try to search for friends using their `email`. You can do this by creating two
different accounts.
...
##### Check if the student successfully implement `location` tracking to determine the
last known `location` of each user.
###### Can users see their friends last known `location` in the map?
###### Was the `location` sent to the server `every 5 seconds` each time the app was
opened?